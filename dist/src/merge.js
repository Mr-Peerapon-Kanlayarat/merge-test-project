...
function merge(collection1, collection2, collection3) {
    ...
    while (a < collection1.length || b < collection2.length || c < collection3.length) {
        const candidates = [];
        if (a < collection1.length)
            candidates.push([collection1[a], 'a']);
        if (b < collection2.length)
            candidates.push([collection2[b], 'b']);
        if (c < collection3.length)
            candidates.push([collection3[c], 'c']);
